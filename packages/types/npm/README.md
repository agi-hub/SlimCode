...
const extension = vscode.extensions.getExtension<RooCodeAPI>("RooVeterinaryInc.SlimCode")
...
**NOTE:** To ensure that the `RooVeterinaryInc.SlimCode` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

```json
"extensionDependencies": ["RooVeterinaryInc.SlimCode"]
```
